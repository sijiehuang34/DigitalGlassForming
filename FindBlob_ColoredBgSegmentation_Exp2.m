...
expImg = 'images\Acquisition500.jpeg';
...
% Create masks
bg_mask_1 = distance_bg1 < threshold; % yellow background
bg_mask_2 = distance_bg2 < threshold; % black background
...
% Define incontinuity threshold (can adjust if needed)
incontThreshold = 200; % pixels

% Compute y differences
dy = diff(sortedY);
...
if ~isempty(jumpIdx)
    % Determine the stop point: the one with the greater y-value in the jump
    if sortedY(jumpIdx) > sortedY(jumpIdx + 1)
        stopIdx = jumpIdx;
    else
        stopIdx = jumpIdx + 1;
    end

    % Trim boundary points beyond (and including) the stop point
...
% Filter out non-stable region (i.e. work zone region)
validIdx = X_vis >= 850;
...
hold on;
plot(X_vis, Z_vis, 'o');
xlabel('X (mm)'); %xlim([5 10]);
ylabel('Z (mm)'); ylim([0 2]);
grid on;
hold off;

% Save the data
saveDir = 'E:\SFF\Exp2_MeasuringTracks_DiffStartsStops\Bad Starts\2_0.5mms_40W_20mm_4mmAboveFocus_PointedTip\';
