%% GlassBlob_vs_LaserData_Exp3_workzone.m
% This program is used to process multiple frames.
clc; clear; close all;

%% Specify the dataset location here
dataDir = 'E:/SFF/Exp3_JoiningTracks/18L_0.5mmsPRINT_40W_20mm_4mmAboveFocus_BlobTip_0mmOverlap_1mmsRETRACT/';
if ~isfolder(dataDir)
...
while idx <= lastValidAcq
    ...
    region1_a = a_channel(700:substrate_start_row-1, 1:region_width);
    region1_b = b_channel(700:substrate_start_row-1, 1:region_width);
...
subplot(1,2,1);
plot(data_nonTrim.timeVector, data_nonTrim.wz_area, 'k.', 'MarkerSize', 15);
xlabel('Time (s)');
ylabel('Area (mm²)');
grid on;
ylim([0 14]);
